...
public abstract class ReactiveModel<T extends SModel>
    implements ReactiveTable<T, SQLiteDatabase>, DoubleConverter<T, Cursor, ContentValues> {
  private Model<T> table = new Model<T>() {
    ...
    public ContentValues to(T t) {
      return ReactiveModel.this.to(t);
    }
  ...
  private abstract class QueryExtras<Q extends S> implements ReactiveModel.Extras<Q>, DoubleConverter<Q, Cursor, ContentValues> {
    ...
    public Maybe<SList<Q>> notIn(final Column<Integer> column, final Integer a, final Integer b) {
      return Maybe.fromCallable(() -> (SList<Q>) table.read(database).notIn(column, a, b)).subscribeOn(Schedulers.from(Promise.instance().executor()))
          .observeOn(Schedulers.from(Promise.instance().executor()));
    }

    @Override
    public Maybe<SList<Q>> like(final Column[] column) {
      return Maybe.fromCallable(() -> (SList<Q>) table.read(database).like(column)).subscribeOn(Schedulers.from(Promise.instance().executor()))
          .observeOn(Schedulers.from(Promise.instance().executor()));
    }

    @Override
    public Maybe<SList<Q>> orderBy(final Column column) {
      return Maybe.fromCallable(() -> (SList<Q>) table.read(database).orderBy(column)).subscribeOn(Schedulers.from(Promise.instance().executor()))
          .observeOn(Schedulers.from(Promise.instance().executor()));
    }

    @Override
    public Maybe<SList<Q>> groupBy(final Column column) {
      return Maybe.fromCallable(() -> (SList<Q>) table.read(database).groupBy(column)).subscribeOn(Schedulers.from(Promise.instance().executor()))
          .observeOn(Schedulers.from(Promise.instance().executor()));
    }

    @Override
    public Maybe<SList<Q>> groupAndOrderBy(final Column column, final Column column1) {
      return Maybe.fromCallable(() -> (SList<Q>) table.read(database).groupAndOrderBy(column, column1)).subscribeOn(Schedulers.from(Promise.instance().executor()))
          .observeOn(Schedulers.from(Promise.instance().executor()));
    }
  }
  ...
  private abstract class QueryExtras2<U extends S> extends QueryExtras<U> {
    ...
    public Maybe<U> first() {
      return Maybe.fromCallable(() -> (U) table.read(database(), columns).first()).subscribeOn(Schedulers.from(Promise.instance().executor()));
    }

    @Override
    public Maybe<U> last() {
      return Maybe.fromCallable(() -> (U) table.read(database(), columns).last()).subscribeOn(Schedulers.from(Promise.instance().executor()))
          .observeOn(Schedulers.from(Promise.instance().executor()));
    ...
    public Maybe<SList<U>> all() {
      return Maybe.fromCallable(() -> (SList<U>) table.read(database(), columns).all()).subscribeOn(Schedulers.from(Promise.instance().executor()))
          .observeOn(Schedulers.from(Promise.instance().executor()));
    }

    @Override
    public Maybe<SList<U>> limit(final int limit) {
      return Maybe.fromCallable(() -> (SList<U>) table.read(database(), columns).limit(limit)).subscribeOn(Schedulers.from(Promise.instance().executor()))
          .observeOn(Schedulers.from(Promise.instance().executor()));
    }

    @Override
    public Maybe<SList<U>> between(final Column<Integer> column, final Integer a, final Integer b) {
      return Maybe.fromCallable(() -> (SList<U>) table.read(database(), columns).between(column, a, b)).subscribeOn(Schedulers.from(Promise.instance().executor()))
          .observeOn(Schedulers.from(Promise.instance().executor()));
    }

    @Override
    public Maybe<SList<U>> where(final Column[] column) {
      return Maybe.fromCallable(() -> (SList<U>) table.read(database(), columns).where(column)).subscribeOn(Schedulers.from(Promise.instance().executor()))
          .observeOn(Schedulers.from(Promise.instance().executor()));
    }

    @Override
    public Maybe<SList<U>> notIn(final Column<Integer> column, final Integer a, final Integer b) {
      return Maybe.fromCallable(() -> (SList<U>) table.read(database(), columns).notIn(column, a, b)).subscribeOn(Schedulers.from(Promise.instance().executor()))
          .observeOn(Schedulers.from(Promise.instance().executor()));
    }

    @Override
    public Maybe<SList<U>> like(final Column[] column) {
      return Maybe.fromCallable(() -> (SList<U>) table.read(database(), columns).like(column)).subscribeOn(Schedulers.from(Promise.instance().executor()))
          .observeOn(Schedulers.from(Promise.instance().executor()));
    }

    @Override
    public Maybe<SList<U>> orderBy(final Column column) {
      return Maybe.fromCallable(() -> (SList<U>) table.read(database(), columns).orderBy(column)).subscribeOn(Schedulers.from(Promise.instance().executor()))
          .observeOn(Schedulers.from(Promise.instance().executor()));
    }

    @Override
    public Maybe<SList<U>> groupBy(final Column column) {
      return Maybe.fromCallable(() -> (SList<U>) table.read(database(), columns).groupBy(column)).subscribeOn(Schedulers.from(Promise.instance().executor()))
          .observeOn(Schedulers.from(Promise.instance().executor()));
    }

    @Override
    public Maybe<SList<U>> groupAndOrderBy(final Column column, final Column column1) {
      return Maybe.fromCallable(() -> (SList<U>) table.read(database(), columns).groupAndOrderBy(column, column1)).subscribeOn(Schedulers.from(Promise.instance().executor()))
          .observeOn(Schedulers.from(Promise.instance().executor()));
    }
  }
}
