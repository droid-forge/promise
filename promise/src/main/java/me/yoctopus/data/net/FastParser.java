...
import me.yoctopus.data.net.extras.InputStreamHttpResponse;
import me.yoctopus.data.net.extras.JsonObjectHttpResponse;
    ...
    .execute(
      new Action<JsonObjectHttpResponse>() {
       ...
       public JsonObjectHttpResponse execute() throws Exception {
        LogUtil.e(TAG, "hitting " + request.toString());
        ...
        response1.status(response.code());
        response1.headers(headers);
        LogUtil.e(TAG, "response" + response1.toString());
        return response1;
       }
      },
